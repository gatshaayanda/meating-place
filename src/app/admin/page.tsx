...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    void Promise.all([getBookingRequests(), getOfferings(), getSpecials()]).then(([requests, menu, offers]) => {
      if (cancelled) return;
      setBookings(requests); setOfferings(menu); setSpecials(offers);
    }).catch(() => { if (!cancelled) setNotice("Operations data could not be loaded. Check Firebase access."); });
    return () => { cancelled = true; };
  }, []);

  const counts = useMemo(() => ({ total: bookings.length, new: bookings.filter((item) => item.status === "New").length, confirmed: bookings.filter((item) => item.status === "Confirmed").length, upcoming: bookings.filter((item) => Boolean(item.date) && String(item.date) >= new Date().toISOString().slice(0, 10) && item.status !== "Cancelled").length }), [bookings]);
  ...
  async function changeStatus(id: string, status: BookingRequestRecord["status"]) {
    try { await updateBookingStatus(id, status); setBookings((items) => items.map((item) => item.id === id ? { ...item, status } : item)); setNotice(`Request marked ${status.toLowerCase()}.`); }
    catch { setNotice("The request status could not be updated."); }
  }
  function startOffering(item?: OfferingRecord) { setDraft(item ? { ...item } : { id: crypto.randomUUID(), name: "", category: "Food", detail: "", price: "", active: true }); }
  async function saveOffering() {
    if (!draft?.name.trim() || !draft.detail.trim()) return;
    try { await saveOfferingRecord(draft); setOfferings((items) => items.some((item) => item.id === draft.id) ? items.map((item) => item.id === draft.id ? draft : item) : [...items, draft]); setDraft(null); setNotice("Offering saved."); }
    catch { setNotice("The offering could not be saved."); }
  }
  async function removeOffering(id: string) {
    try { await deleteOfferingRecord(id); setOfferings((items) => items.filter((item) => item.id !== id)); setNotice("Offering removed."); }
    catch { setNotice("The offering could not be removed."); }
  }
  async function saveSpecial() {
    if (!specialDraft?.title.trim() || !specialDraft.detail.trim()) return;
    try { await saveSpecialRecord(specialDraft); setSpecials((items) => items.some((item) => item.id === specialDraft.id) ? items.map((item) => item.id === specialDraft.id ? specialDraft : item) : [...items, specialDraft]); setSpecialDraft(null); setNotice("Special saved."); }
